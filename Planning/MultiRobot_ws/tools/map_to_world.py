...
<!--
  ...
  Frame      : deskewed by {angle:.1f} deg, extent {extent}.
...
def emit_world(rects, res, origin_xy, height, name, source, angle, extent,
    ...
    return WORLD_TEMPLATE.format(
        name=name, source=source, angle=angle, extent=extent, provenance=provenance,
...
def main(argv=None):
    ...
    parser.add_argument('--provenance',
                        default='Willow Garage office, mapped 2010, distributed with '
                                'turtlebot_navigation (BSD).',
                        help='one line on where the source map came from, copied '
                             'into the generated world header')
    ...
    world = emit_world(
        ...
        float(free.sum()) * res * res, args.rtf, args.rate, args.provenance)
